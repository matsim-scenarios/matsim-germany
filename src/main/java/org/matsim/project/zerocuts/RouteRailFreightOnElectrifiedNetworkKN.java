...
import static org.matsim.prepare.CreateNetworkFromOSM.*;
import static org.matsim.prepare.longDistanceFreightGER.GenerateFreightPlans.*;

public class RouteRailFreightOnElectrifiedNetworkKN implements MATSimAppCommand {
...
//		defaultValue = "../shared-svn/projects/matsim-germany/german-wide-freight-v3/before-calibration/german-wide-freight-v3-network.xml.gz")
	...
	private static final Logger log = LogManager.getLogger(RouteRailFreightOnElectrifiedNetworkKN.class);
	...
	public Integer call() throws Exception {
		...
		Set<String> railwayModes = Set.of(OsmTags.RAIL, RAIL_ELECTRIFIED, RAIL_ELECTRIFIED_INCL_PROPOSED);
		...
		config.scoring().addActivityParams( new ActivityParams("freight_start").setTypicalDuration(3600 ) );
		config.scoring().addActivityParams( new ActivityParams("freight_end").setTypicalDuration(3600 ) );

		config.scoring().addModeParams( new ModeParams(LEG_MODE_FREIGHT_RAIL).setMarginalUtilityOfTraveling(0.0 ).setMonetaryDistanceRate(-0.1 ) );
		config.scoring().addModeParams( new ModeParams( RAIL_ELECTRIFIED).setMarginalUtilityOfTraveling(0.0 ).setMonetaryDistanceRate(-0.1 ) );
		config.scoring().addModeParams( new ModeParams( RAIL_ELECTRIFIED_INCL_PROPOSED).setMarginalUtilityOfTraveling(0.0 ).setMonetaryDistanceRate(-0.1 ) );
		...
		for (String mode: modesThatNeedVehicleTypes) {
			VehicleType vehicleType = scenario.getVehicles().addModeVehicleType(mode).setMaximumVelocity(80/3.6);
		...
		for (Person person: scenario.getPopulation().getPersons().values()) {
			...
			Person electrifiedRoutePerson = pf.createPerson(Id.createPersonId(person.getId().toString() + "_electrified"));
			Plan electrifiedRoutePlan = pf.createPlan();
			electrifiedRoutePerson.addPlan(electrifiedRoutePlan);
			personsToAdd.add(electrifiedRoutePerson);

			Person electrifiedInclProposedRoutePerson = pf.createPerson(Id.createPersonId(person.getId().toString() + "_electrifiedInclProposed"));
			Plan electrifiedInclProposedRoutePlan = pf.createPlan();
			...
			for (PlanElement planElement: person.getSelectedPlan().getPlanElements()) {
				if (planElement instanceof Activity) {
					...
					electrifiedRoutePlan.addActivity(pf.createActivityFromCoord(activity.getType(), activity.getCoord()));
					electrifiedInclProposedRoutePlan.addActivity(pf.createActivityFromCoord(activity.getType(), activity.getCoord()));
				} else {
					electrifiedRoutePlan.addLeg(pf.createLeg( RAIL_ELECTRIFIED ) );
					electrifiedInclProposedRoutePlan.addLeg(pf.createLeg( RAIL_ELECTRIFIED_INCL_PROPOSED ) );
				}
			}
		}
		...
		final VehiclesFactory vf = scenario.getVehicles().getFactory();
		for (Person person: scenario.getPopulation().getPersons().values()) {
//			VehicleUtils.insertVehicleTypesIntoPersonAttributes(person, modeToVehicleType);
			...
			for (Map.Entry<String, Id<VehicleType>> modeVehicleType: modeToVehicleType.entrySet()) {
				...
				Vehicle vehicle = vf.createVehicle(vehicleId, scenario.getVehicles().getVehicleTypes().get(modeVehicleType.getValue() ) );
...
//		injector.getInstance( PrepareForSim.class ).run();
		...
		filterElectrified.addLinkFilter(l -> {
			if (l.getAllowedModes().contains( RAIL_ELECTRIFIED )) {
				return true;
			}
			return false;
		});
		...
		CSVPrinter printer = new CSVPrinter(IOUtils.getBufferedWriter(output.toString() ), CSVFormat.Builder.create().setDelimiter( "," ).setHeader( HEADER ).get() );
		...
		Counter counter1 = new Counter( "routed person # " );
		for (Person person: scenario.getPopulation().getPersons().values()) {
			...
			Facility destFacility = FacilitiesUtils.wrapActivity(trip.getDestinationActivity());
			...
			List<? extends PlanElement> routeNonElectrified = tripRouter.calcRoute( OsmTags.RAIL, originFacility, destFacility, 0.0d, person, trip.getTripAttributes() );
			...
			List<? extends PlanElement> routeElectrified = tripRouter.calcRoute( RAIL_ELECTRIFIED, originFacility, destFacility, 0.0d, electrifiedRoutePerson, trip.getTripAttributes() );
			Leg electrifiedLeg = (Leg) routeElectrified.get(2);

			List<? extends PlanElement> routeElectrifiedInclProposed = tripRouter.calcRoute( RAIL_ELECTRIFIED_INCL_PROPOSED, originFacility, destFacility, 0.0d, electrifiedInclProposedRoutePerson, trip.getTripAttributes() );
			...
			try {
				...
				printer.print(destFacility.getCoord().getX());
				printer.print(destFacility.getCoord().getY());
				printer.print(originFacility.getLinkId());
				printer.print(destFacility.getLinkId());
		...
		// ===
		log.error("stopping here for debugging");
		if ( true ){
			return 0;
		}
		...
		if (!outputRun.equals("")) {
			...
			config.replanning().addStrategySettings(
				new StrategySettings().setStrategyName("ChangeExpBeta" ).setWeight(1.0 ).setSubpopulation("longDistanceFreight" ) );
